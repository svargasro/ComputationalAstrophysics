...
def graphicsRK4(x,y,t,energy,angularMom,isAdaptative):
    ...
    if isAdaptative:
        ...
        for i in range(num_points):
            axs[0].axvline(x=t[i], color='gray', linestyle='-', alpha=0.5)
        axs[0].set_xlabel('t')
        axs[0].set_ylabel('x')
        ...
        for i in range(num_points):
            axs[1].axvline(x=t[i], color='gray', linestyle='-', alpha=0.5)
        axs[1].set_xlabel('t')
        axs[1].set_ylabel('y')
...
def main():
    ...
    y0,vx0 = 0,0
    finalTime = 49.5*5 #Periodo es aproximadamente 49.5 años
    dt = 0.0002 #0.0002
